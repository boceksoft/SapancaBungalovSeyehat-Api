...
class Faq
{
    ...
    public static function Index($Routing)
    {
        ...
        global $qsql;
        global $url;
        $query = $db->prepare("select id,title,baslik,left(description,250) as description,'/'+url as url,cat,replace(icerik,'../../../../..','https://cdn.sapancabungalovseyahat.com') as icerik, case when len(canonical) > 0 then  concat('{$qsql['domain']}/',canonical) else '{$qsql['domain']}/".implode("/",$url)."' end  as canonical from sayfalar where id=:id");
