...
class Page
{
    ...
    public static function Detail($Routing){
        ...
        $query = $db->prepare("select id,title,isnull(resim, '') as resim, kapak,baslik,left(description,250) as description,'/'+url as url,cat,replace(icerik,'../../../../..','https://cdn.sapancabungalovseyahat.com') as icerik, case when len(canonical) > 0 then  concat('{$qsql['domain']}/',canonical) else '{$qsql['domain']}/".implode("/",$url)."' end  as canonical from sayfalar".UZANTI." where id=:id");
        ...
        $query = $db->prepare("select id,title,resim,baslik,left(description,250) as description,url,replace(icerik,'../../../../..','https://cdn.sapancabungalovseyahat.com') as icerik, case when len(canonical) > 0 then  concat('{$qsql['domain']}/',canonical) else '{$qsql['domain']}/".implode("/",$url)."' end  as canonical from sayfalar".UZANTI." where not blog=1 and aktif=1 and cat=:cat");
    ...
    public static function About($Routing){
        ...
        global $qsql;
        global $url;
        $query = $db->prepare("select id,title,kisa_icerik,baslik,kapak,resim,left(description,250) as description,'/'+url as url,cat,replace(icerik,'../../../../..','https://cdn.sapancabungalovseyahat.com') as icerik, case when len(canonical) > 0 then  concat('{$qsql['domain']}/',canonical) else '{$qsql['domain']}/".implode("/",$url)."' end  as canonical from sayfalar where id=:id");
        $query->execute(["id"=>$Routing["EntityId"]]);
        $result = $query->fetch(PDO::FETCH_ASSOC);
        ...
        $query = $db->prepare("select id,title,resim,baslik,left(description,250) as description,url,replace(icerik,'../../../../..','https://cdn.sapancabungalovseyahat.com') as icerik, case when len(canonical) > 0 then  concat('{$qsql['domain']}/',canonical) else '{$qsql['domain']}/".implode("/",$url)."' end  as canonical from sayfalar where not blog=1 and aktif=1 and cat=:cat");
        $query->execute(["cat"=>$result["cat"]=="0" ? $Routing["EntityId"] : $result["cat"] ]);
        $result["_pages"]=$query->fetchAll(PDO::FETCH_ASSOC);
